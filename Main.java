	...
	public static void main(String[] args) {
		...
		for(int r=0; r<(int)options.get(CONSTANTS.REPETITION); r++) {
			...
			for (VehicleRoute route : solution.getRoutes()) {
				System.out.println();
				System.out.println(route.getVehicle().getType().getCapacityDimensions());
				System.out.println(analyser.getMaxLoad(route));
				System.out.println(analyser.getLoadAtEnd(route));
				System.out.println("");
				System.out.println("getDistance "+analyser.getDistance(route));
				System.out.println("getWaitingTime "+analyser.getWaitingTime(route));
				System.out.println("getServiceTime "+analyser.getServiceTime(route));
				System.out.println("getTransportTime "+analyser.getTransportTime(route));
				System.out.println("getFixedCosts "+analyser.getFixedCosts(route));
				System.out.println("getVariableTransportCosts "+analyser.getVariableTransportCosts(route));

				totalCost += analyser.getDistance(route);// + analyser.getWaitingTime(route) + analyser.getServiceTime(route);
